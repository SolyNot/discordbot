...
TASKS_POOL = [
    ...
    {"type":"general", "text":"Start a short discussion in #general (post + at least one reply from anyone)","channel": GENERAL_CHANNEL_ID}
]
...
class TaskView(View):
    ...
    async def verify_button(self, interaction: discord.Interaction, button: Button):
        ...
        if entry.get("key_given"):
            await interaction.response.send_message("You already received the key.", ephemeral=True)
            return
...
async def getkey(interaction: discord.Interaction):
    ...
    if entry and entry.get("assigned") and not entry.get("completed"):
        ...
        if not channel:
            await interaction.followup.send("Can't access verification channel.", ephemeral=True)
            return
        v = await verify_user_posted(channel, interaction.user.id, entry["assigned_at"], entry["type"])
        if v["ok"]:
            entry["completed"] = True
